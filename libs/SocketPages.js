...
-----------------------------------------------------------------------------------------*/
var SocketPages = function SocketPages(app, socket) {
  this.app = app;
  this.socket = socket;
...
function subscribe(io, socket, data, SocketHandler) {
  ...
  if (typeof self.app.CACHE.rooms[room] !== "undefined" && typeof self.app.CACHE.rooms[room]["editing"] !== "undefined" && self.app.CACHE.rooms[room]["editing"].filter(function (sid) {
    return socketId != sid;
  }).length) {
    setTimeout(function () {
      socket.emit('wikiPageCtrl_editing', { isEditing: true, who: self.app.CACHE.sockets[self.app.CACHE.rooms[room]["editing"].filter(function (sid) {
          return socketId != sid;
        })[0]] });
    }, 500);
  }
    ...
    if (!editState) {
      ...
      if (this.app.CACHE.rooms[room]["editing"].length) {
        var oUsersEditing = _.omit(this.app.CACHE.sockets, function (val, key, o) {
          return !self.app.CACHE.rooms[room]["editing"].filter(function (sid) {
            return sid == key;
          }).length;
        });
