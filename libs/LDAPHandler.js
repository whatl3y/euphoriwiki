...
-----------------------------------------------------------------------------------------*/
var LDAPHandler = function LDAPHandler(options) {
  options = options || {};
...
LDAPHandler.prototype.find = function (options, cb) {
  ...
  if (!query && !value) {
    ...
    try {
      if (query) this.ad.find({ timeout: timeout, filter: query }, cb);else this.ad.find({ timeout: timeout, filter: attribute + "=" + value }, cb);
    } catch (err) {
      cb(err);
    }
  }
