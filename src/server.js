...
try {
  ...
  if (config.server.CLUSTERING) {
    if (!sticky.listen(httpServer, config.server.PORT)) {
      //if (cluster.isMaster) {}
      ...
      cpuCount =
        cpuCount > config.server.CLUSTER_MAX_CPUS
          ? config.server.CLUSTER_MAX_CPUS
          : cpuCount
    ...
    [
      ...
      function(callback) {
        new WikiHandler().initQueries(function(err, data) {
          log.debug(
            'Finished initialization tasks in WikiHandler().initQueries().'
          )
          callback(err, data)
        })
      },
    ...
    function(err, results) {
      ...
      queries.forEach((queryInfo) => {
        if (queryInfo.extracode) {
          try {
            ...
            log.debug(
              `Successfully ran extra code for initializequeries - collection: ${
            ...
            log.error(
              err,
              `Error running extra code for initializequeries - collection: ${
                queryInfo.collection
              } info: ${JSON.stringify(queryInfo)}`
            )
          }
      ...
      _.each(oData.routes, function(route) {
        try {
          ...
          log.error(
            err,
            'Error binding route to express; method: ' +
              route.verb +
              '; path: ' +
              route.path
          )
      ...
      //passport setup
      _.each(fs.readdirSync('./passport_strategies') || [], function(
        stratFile
      ) {
        try {
