...
|PURPOSE:
|EXAMPLE CALLS:
|        A full screen, synchronous loader:
|          new Core.Modals().alertPopup({loading:true});
|          $( '#loader' ).remove();
|
|        A small loader with text at the bottom of the page as work happens:
|          var loader = new Core.Modals().asyncLoader({message:"Some text to show the user..."});
|          loader.remove();
|
...
Core.Modals = function() {
  //Class for Core.Modals and Loading popups
}
